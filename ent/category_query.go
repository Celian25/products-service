...
func (_q *CategoryQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Category, error) {
	...
	for name, query := range _q.withNamedProducts {
		if err := _q.loadProducts(ctx, query, nodes,
			func(n *Category) { n.appendNamedProducts(name) },
			func(n *Category, e *Product) { n.appendNamedProducts(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *CategoryQuery) loadProducts(ctx context.Context, query *ProductQuery, nodes []*Category, init func(*Category), assign func(*Category, *Product)) error {
	...
	for _, n := range neighbors {
		fk := n.product_category
		if fk == nil {
			return fmt.Errorf(`foreign-key "product_category" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "product_category" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (_q *CategoryQuery) WithNamedProducts(name string, opts ...func(*ProductQuery)) *CategoryQuery {
	query := (&ProductClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
