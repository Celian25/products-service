...
type FilterQuery struct {
	...
	withFilterType    *FilterTypeQuery
	withProducts      *ProductQuery
	withFKs           bool
	modifiers         []func(*sql.Selector)
	loadTotal         []func(context.Context, []*Filter) error
...
// QueryFilterType chains the current query on the "filter_type" edge.
func (_q *FilterQuery) QueryFilterType() *FilterTypeQuery {
...
func (_q *FilterQuery) QueryProducts() *ProductQuery {
...
func (_q *FilterQuery) Clone() *FilterQuery {
	...
	return &FilterQuery{
		...
		withFilterType: _q.withFilterType.Clone(),
...
// WithFilterType tells the query-builder to eager-load the nodes that are connected to
// the "filter_type" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *FilterQuery) WithFilterType(opts ...func(*FilterTypeQuery)) *FilterQuery {
	query := (&FilterTypeClient{config: _q.config}).Query()
...
func (_q *FilterQuery) WithProducts(opts ...func(*ProductQuery)) *FilterQuery {
	query := (&ProductClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *FilterQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Filter, error) {
	var (
		...
		_spec       = _q.querySpec()
		loadedTypes = [2]bool{
			_q.withFilterType != nil,
			_q.withProducts != nil,
		}
	)
	if _q.withFilterType != nil {
		withFKs = true
	}
	if withFKs {
		_spec.Node.Columns = append(_spec.Node.Columns, filter.ForeignKeys...)
	}
	...
	if query := _q.withFilterType; query != nil {
		if err := _q.loadFilterType(ctx, query, nodes, nil,
			func(n *Filter, e *FilterType) { n.Edges.FilterType = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withProducts; query != nil {
		if err := _q.loadProducts(ctx, query, nodes,
			func(n *Filter) { n.Edges.Products = []*Product{} },
			func(n *Filter, e *Product) { n.Edges.Products = append(n.Edges.Products, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range _q.withNamedProducts {
		if err := _q.loadProducts(ctx, query, nodes,
			func(n *Filter) { n.appendNamedProducts(name) },
			func(n *Filter, e *Product) { n.appendNamedProducts(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *FilterQuery) loadFilterType(ctx context.Context, query *FilterTypeQuery, nodes []*Filter, init func(*Filter), assign func(*Filter, *FilterType)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Filter)
	for i := range nodes {
		if nodes[i].filter_filter_type == nil {
			continue
		}
		fk := *nodes[i].filter_filter_type
	...
	query.Where(filtertype.IDIn(ids...))
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "filter_filter_type" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *FilterQuery) loadProducts(ctx context.Context, query *ProductQuery, nodes []*Filter, init func(*Filter), assign func(*Filter, *Product)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(filter.ProductsTable)
		s.Join(joinT).On(s.C(product.FieldID), joinT.C(filter.ProductsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(filter.ProductsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(filter.ProductsPrimaryKey[1]))
	...
	qr := QuerierFunc(func(ctx context.Context, q Query) (Value, error) {
		return query.sqlAll(ctx, func(_ context.Context, spec *sqlgraph.QuerySpec) {
			...
			spec.Assign = func(columns []string, values []any) error {
				...
				if nids[inValue] == nil {
					nids[inValue] = map[*Filter]struct{}{byID[outValue]: {}}
...
func (_q *FilterQuery) WithNamedProducts(name string, opts ...func(*ProductQuery)) *FilterQuery {
	query := (&ProductClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if _q.withNamedProducts == nil {
		_q.withNamedProducts = make(map[string]*ProductQuery)
	}
