...
func (c *Client) noder(ctx context.Context, table string, id int) (Noder, error) {
	...
	case filter.Table:
		query := c.Filter.Query().
			Where(filter.ID(id))
		if fc := graphql.GetFieldContext(ctx); fc != nil {
			if err := query.collectField(ctx, true, graphql.GetOperationContext(ctx), fc.Field, nil, filterImplementors...); err != nil {
				return nil, err
			}
		}
		return query.Only(ctx)
	case filtertype.Table:
		query := c.FilterType.Query().
			Where(filtertype.ID(id))
		if fc := graphql.GetFieldContext(ctx); fc != nil {
			if err := query.collectField(ctx, true, graphql.GetOperationContext(ctx), fc.Field, nil, filtertypeImplementors...); err != nil {
				return nil, err
			}
		}
		return query.Only(ctx)
...
func (c *Client) noders(ctx context.Context, table string, ids []int) ([]Noder, error) {
	...
	case filter.Table:
		query := c.Filter.Query().
			Where(filter.IDIn(ids...))
		query, err := query.CollectFields(ctx, filterImplementors...)
		if err != nil {
			return nil, err
		}
		nodes, err := query.All(ctx)
	...
	case filtertype.Table:
		query := c.FilterType.Query().
			Where(filtertype.IDIn(ids...))
		query, err := query.CollectFields(ctx, filtertypeImplementors...)
		if err != nil {
			return nil, err
		}
		nodes, err := query.All(ctx)
