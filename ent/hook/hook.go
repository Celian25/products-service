...
// The FilterTypeFunc type is an adapter to allow the use of ordinary
// function as FilterType mutator.
type FilterTypeFunc func(context.Context, *ent.FilterTypeMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f FilterTypeFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.FilterTypeMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.FilterTypeMutation", m)
}
