...
func (_c *ProductCreate) SetPhotoURL(v string) *ProductCreate {
	_c.mutation.SetPhotoURL(v)
	return _c
}

// SetNillablePhotoURL sets the "photo_url" field if the given value is not nil.
func (_c *ProductCreate) SetNillablePhotoURL(v *string) *ProductCreate {
	if v != nil {
		_c.SetPhotoURL(*v)
	}
	return _c
...
func (_c *ProductCreate) AddFilterIDs(ids ...int) *ProductCreate {
	_c.mutation.AddFilterIDs(ids...)
	return _c
}

// AddFilters adds the "filters" edges to the Filter entity.
func (_c *ProductCreate) AddFilters(v ...*Filter) *ProductCreate {
	ids := make([]int, len(v))
	for i := range v {
		ids[i] = v[i].ID
	}
