...
func (_q *ProductQuery) QueryFilters() *FilterQuery {
...
func (_q *ProductQuery) WithFilters(opts ...func(*FilterQuery)) *ProductQuery {
	query := (&FilterClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ProductQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Product, error) {
	...
	if query := _q.withFilters; query != nil {
		if err := _q.loadFilters(ctx, query, nodes,
			func(n *Product) { n.Edges.Filters = []*Filter{} },
			func(n *Product, e *Filter) { n.Edges.Filters = append(n.Edges.Filters, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range _q.withNamedFilters {
		if err := _q.loadFilters(ctx, query, nodes,
			func(n *Product) { n.appendNamedFilters(name) },
			func(n *Product, e *Filter) { n.appendNamedFilters(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *ProductQuery) loadCategory(ctx context.Context, query *CategoryQuery, nodes []*Product, init func(*Product), assign func(*Product, *Category)) error {
	...
	for i := range nodes {
		if nodes[i].product_category == nil {
			continue
		}
		fk := *nodes[i].product_category
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "product_category" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *ProductQuery) loadFilters(ctx context.Context, query *FilterQuery, nodes []*Product, init func(*Product), assign func(*Product, *Filter)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(product.FiltersTable)
		s.Join(joinT).On(s.C(filter.FieldID), joinT.C(product.FiltersPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(product.FiltersPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(product.FiltersPrimaryKey[0]))
	...
	neighbors, err := withInterceptors[[]*Filter](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
...
func (_q *ProductQuery) WithNamedFilters(name string, opts ...func(*FilterQuery)) *ProductQuery {
	query := (&FilterClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
